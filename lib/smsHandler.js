...
handlers.sendTwilloSms = (phoneNumber, message, callback) => {

    const phoneNumber = String(phoneNumber) && phoneNumber.trim().length == 10 ? phoneNumber.trim() : false

    const message = String(message) && message.trim().length > 0 && message.trim().length < 100 ? message.trim() : false
